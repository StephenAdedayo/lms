      ...
      <table className='md:table-auto table-fixed w-full overflow-hidden border border-gray-500/20 mt-10'>
        ...
        <tbody className='text-gray-700'>
          {enrolledCourses.map((course, index) => (
            <tr key={index} className='border-b border-b-gray-500/20'>
              ...
              <td className='px-4 py-3 max-sm:text-right'>
                <button onClick={() => navigate(`/player/${course._id}`)} className='px-3 sm:px-5 py-1.5 sm:py-2  bg-blue-600 max-sm:text-xs text-white'>{progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? "Completed" : "On Going"}</button>
              </td>
            </tr>
          ))}
        </tbody>
